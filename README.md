...
- 깊이 정렬, 지면 그림자와 LPC 이동·공격·점프 애니메이션 렌더링

공격 피해, 체력, 사망, 재생성, 클라이언트 예측과 보간은 아직 포함하지 않습니다.
...
공격 지속시간은 모두 서버가 결정합니다. 클라이언트는 연속된 서버 스냅숏의 좌표 차이로
상·하·좌·우 애니메이션 방향을 고르고, 서버의 `Z`와 공격 상태에 맞춰 LPC 점프·공격
프레임을 재생합니다.
...
- X 공격은 방향과 범위만 표시하며 피해 판정은 아직 없습니다.
