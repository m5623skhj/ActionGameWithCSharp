...
- X 공격, C 점프, 서버 판정 HP/피해/사망 처리
- 향후 동작을 위한 Z 입력 예약
- 두 클라이언트의 플레이어 위치 동기화
- 깊이 정렬, 지면 그림자와 LPC 이동·공격·점프·사망 애니메이션 및 HP 바 렌더링

각 캐릭터는 HP 100으로 시작하며, 공격 범위 안의 상대에게 한 번의 공격당 20의 피해를
줍니다. HP가 0이 되면 사망 상태가 유지되고 이동, 점프, 공격 입력은 무시됩니다.
재생성과 클라이언트 예측 및 보간은 아직 포함하지 않습니다.
...
공격 지속시간과 피해는 모두 서버가 결정합니다. 공격 판정은 바라보는 방향의 X 거리와
Y축 깊이 차이, 점프 높이 차이를 함께 검사합니다. 클라이언트는 연속된 서버 스냅숏의
좌표 차이로 상·하·좌·우 애니메이션 방향을 고르고, 서버의 `Z`, 공격 및 사망 상태에
맞춰 LPC 프레임을 재생합니다.
