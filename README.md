...
4초이며 서버가 검증합니다. 바라보는 방향을 기준으로 `↓ → Z`를 0.6초 안에 차례로
입력해도 같은 전용 스킬이 발동합니다. 화면 왼쪽 아래 게이지와 Z 표시로 준비 상태를
확인할 수 있습니다.
...
- 현재 커맨드 큐에는 캐릭터별 전용 스킬 하나만 등록되어 있습니다.
